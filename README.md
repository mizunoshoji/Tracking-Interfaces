...
## リリースビルド（App Store Connect）

EAS Build を使う。初回のみ `eas login` と `eas build:configure` が必要。
...
クラウドでビルドが走り、完了すると `.ipa` のダウンロードリンクが表示される。
...
`--latest` は直近のビルドを自動で選択する。**これが失敗した場合**は下記で代替する。
...
あるいは `.ipa` をダウンロードして **Transporter**（Mac App Store）や **Xcode → Window → Organizer → Distribute App** から手動アップロードする。
